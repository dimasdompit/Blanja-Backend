module.exports = {
  ...
  getTransactionDetails: `SELECT tr.*, p.products as productName, p.price as productPrice, s.size, c.color, p.image as productImage FROM transaction_detail tr JOIN products p ON p.id = tr.product_id JOIN sizes s ON s.id = p.size_id JOIN colors c ON c.id = p.color_id WHERE tr.transaction_id = ?`,
